...
| Rice preparation | `rice_panicle` -> `rice` + `straw` through `create:milling`; `rice` is the sole current `c:crops/rice` member and feeds Farmer's Delight cooking. | Concrete agriculture -> Create -> food bridge. Static contract and focused Millstone observation both passed. |
